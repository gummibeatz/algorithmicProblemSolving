...
public class TrashRemoval
{
	...
	// returns minimum of all the max lengths for each pt.
	public static double solve(ArrayList<Point> chullp)
	{
		double minOfMax = Double.POSITIVE_INFINITY;
		for(int i=0; i<chullp.size(); i++)
		{
			int pt3ct = (i+2)%chullp.size();
			Point pt1 = chullp.get(i);
			Point pt2 = chullp.get((i+1)%chullp.size());
			double max = 0;
			//getting max
			Point pt3 = chullp.get(pt3ct);
			while(!pt3.equals(pt1))
			{
				double height = height(pt1,pt2,pt3);
				if(height>max) max = height;
				pt3ct = (pt3ct+1)%chullp.size();
				pt3 = chullp.get(pt3ct);
			}
			if(max<minOfMax) minOfMax = max;
		}
		return(format(minOfMax));
	}
	//formats double to round up to the nearest hundredth
	public static double format(double d) {return Math.ceil(d*100)/100;}
	...
	public static void main(String[] args) throws IOException
	{
		...
		while((line=in.readLine())!=null)
		{
			...
			chullp = chull(points);	
			sb.append("Case ").append(counter).append(": ").append(df.format(solve(chullp))).append("\n");			
		}
		System.out.print(sb);
	}
